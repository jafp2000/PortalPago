...
class Cobros extends Model
{
    ...
    public static function ListaCobrosFiltrosPagados($idUsuario, $fecha_vencimiento, $empresa, $buscar_rut, $vencimiento, $monto)
    {
        ...
        */
                    ...
                    $query = "(SELECT C.idCobros,C.empresa,C.descripcion,C.fecha_vencimiento,C.monto,C.fecha_pago,C.nro_transaccion,1 AS tipo,C.rut_empresa as rut,C.url_adjunto as 'adjunto', T.descripcion as forma_pago
                    FROM cobros C
                    INNER JOIN usuarios U ON C.idUsuarios = U.idUsuarios
                    LEFT JOIN tipo_pagos T ON C.idTipoPago = T.idTipoPago
                    WHERE C.eliminado = 0 AND pagado = 1 AND C.idUsuarios = ".$idUsuario.
                    " UNION ALL
                    SELECT D.idnominasdetalle,D.nombre AS empresa,D.descripcion,D.fecha_vencimiento,D.monto,D.fecha_pago,D.nro_transaccion,2 AS tipo,D.rut,D.idnominas as 'adjunto',T.descripcion as forma_pago
                    FROM nominas N
                    INNER JOIN nominasdetalle D on N.idnominas = D.idnominas
                    INNER JOIN usuarios U ON N.idUsuarios = U.idUsuarios
                    LEFT JOIN tipo_pagos T ON D.idTipoPago = T.idTipoPago
                    WHERE N.eliminado = 0 AND pagado = 1 AND N.idUsuarios = ".$idUsuario." )";

                    /*$data = DB::table(DB::raw($query))
                    ->setBindings(array($idUsuario, $idUsuario));*/

                    $first = DB::table('cobros')
                    ->select(
                        'cobros.idCobros',
                        'cobros.empresa',
                        'cobros.descripcion',
                    ...
                    ->join('usuarios', 'cobros.idUsuarios', '=', 'usuarios.idUsuarios')
                    ->leftJoin('tipo_pagos', 'cobros.idTipoPago', '=', 'tipo_pagos.idTipoPago')
                    ->where([
                       ['cobros.eliminado', '=', '0'],
                       ['cobros.pagado', '=', '1'],
                       ['cobros.idUsuarios', '=', $idUsuario]
                   ]);

                    $data = DB::table('nominas')
                    ->select(
                        'nominasdetalle.idnominasdetalle',
                        'nominasdetalle.nombre',
                        'nominasdetalle.descripcion',
                        'nominasdetalle.fecha_vencimiento',
                        'nominasdetalle.monto',
                        'nominasdetalle.fecha_pago',
                        'nominasdetalle.nro_transaccion',
                        'nominasdetalle.rut',
                        'nominasdetalle.idnominas',
                        'tipo_pagos.descripcion'
                    )
                    ->join('nominasdetalle', 'nominas.idnominas', '=', 'nominasdetalle.idnominas')
                    ->join('usuarios', 'nominas.idUsuarios', '=', 'usuarios.idUsuarios')
                    ->leftJoin('tipo_pagos', 'detallenominas.idTipoPago', '=', 'tipo_pagos.idTipoPago')
                    ->where([
                        ['nominas.eliminado', '=', '0'],
                        ['nominasdetalle.pagado', '=', '1'],
                        ['nominas.idUsuarios', '=', $idUsuario]
                    ])
                    ->union($first)
                    ->get();
